...
export async function notionApi(endpoint: string, body: {}) {
  ...
  }).catch((err) => console.log('from notionApi ' + err));

  if (!res || !res.ok) {
    console.log('from if statement form notionApi ' + res);
  }
...
export async function getNewItems(): Promise<NewItem[]> {
  const notionData = await notionApi(
    `/databases/${process.env.NOTION_DB_ID}/query`,
    {
      filter: {
        property: 'Status',
        status: {
